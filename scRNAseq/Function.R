...
ScRNA_data_processing <- function(object,resolution=c(0.5), 
                                  reduction=c("pca","SCT","harmony"),group.by =NULL,
                                  regress=NULL,verbose = T,
                                  nfeatures=3000, dims=1:30)
{
  library(Seurat)
  ...
  if(reduction %in% c("pca","harmony")){
      ...
      object <- FindVariableFeatures(object = object,nfeatures=nfeatures,verbose=verbose)
      object <- ScaleData(object = object,features = VariableFeatures(object),
                        vars.to.regress = regress,verbose=verbose)
      object <- RunPCA(object=object,features=VariableFeatures(object),
                       verbose=verbose)
  }
  
  if (reduction %in% "SCT") {
      print("SCT Normalized~")
  ...
  if (reduction %in% "harmony") {
      library(harmony)
      print("Harmony integration!!")

      object <- RunHarmony(object,group.by.vars = group.by,
                           plot_convergence = T,verbose=verbose)

      object <- RunUMAP(object = object,dims=dims,reduction="harmony",verbose=verbose)
      object <- FindNeighbors(object = object,dims=dims,reduction="harmony",verbose=verbose)
  }else{
    object <- RunUMAP(object = object,dims=dims,verbose=verbose)
    object <- FindNeighbors(object = object,dims=dims,verbose=verbose)
  }
  
  for (i in resolution) {

      object <- FindClusters(object = object,resolution = i,verbose=verbose)
  }
